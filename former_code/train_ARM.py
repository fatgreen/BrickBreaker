...
class DDQNAgent:
    def __init__(self, input_shape, num_actions=3, lr=1e-4, epsilon=1.0, epsilon_decay=0.995, epsilon_min=0.01, gamma=0.99, batch_size=32, buffer_size=1e4):
        ...
        hidden_size = self.q_network.arm.U.out_features
        feat_dim    = self.q_network.conv(torch.zeros(1,1,screen_height,screen_width)).shape[1]
        self.reset_arm_state(hidden_size, feat_dim)
    ...
    def train(self):
        ...
        state, actions, rewards, next_state, dones = zip(*batch)

        # 將列表中的 sequence（長度=4，每個 frame shape: (1, 1, H, W)）轉換為 tensor
        # 組成的 shape: (batch_size, seq_len, 1, H, W)
        # states: tuple of (1,1,H,W), next_states 同理
        state = torch.cat(state, dim=0).to(self.device)        # (B,1,H,W)
        ...
        # DDQN 計算
        q_values = self.q_network(state).gather(1, actions)
        next_actions = torch.argmax(self.q_network(next_state), dim=1, keepdim=True)
...
#     agent.target_network.eval()
    ...
    for episode in range(N_EPISODES):
        ...
        for step in range(MAX_TIMESTEPS):
            ...
            action = agent.select_action(current_tensor)
            next_frame, reward, done, dead = env.step(action)
            next_tensor = agent.preprocess_frame(next_frame)
            agent.store_experience(current_tensor, action, reward, next_tensor, done)
